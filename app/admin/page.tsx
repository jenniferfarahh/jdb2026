...
export default function AdminPage() {
  ...
  const exportCSV = () => {
    ...
    for (const v of data.votes) {
      const ps = v.projectVotes.map(p => {
        const proj = projets.find(x => x.id === p.projectId);
        return `#${p.rank} ${proj?.name ?? p.projectId}(${p.weight}pts)`;
      }).join(" | ");
      const os = v.ongVotes.map(o => {
        const ong = ongs.find(x => x.id === o.ongId);
        return `#${o.rank} ${ong?.name ?? o.ongId}(${o.weight}pts)`;
      }).join(" | ");
    ...
    a.href = url; a.download = "jdb2026_votes.csv"; a.click();
  ...
  if (!secret) {
    return (
      <div style={{ minHeight: "100dvh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
        <div className="glass" style={{ borderRadius: 24, padding: "36px 32px", maxWidth: 420, width: "100%", textAlign: "center" }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🔐</div>
          <h1 style={{ fontSize: "1.8rem", fontWeight: 900, color: "var(--text)", marginBottom: 8 }}>Admin JDB 2026</h1>
  ...
  const filtered = (data?.votes ?? []).filter(v =>
    !search || v.prenom.toLowerCase().includes(search.toLowerCase()) ||
    v.promoType.toLowerCase().includes(search.toLowerCase())
  );

  const byPromo: Record<string, number> = {};
  for (const v of data?.votes ?? []) {
    byPromo[v.promoType] = (byPromo[v.promoType] ?? 0) + 1;
  }
    ...
    <div style={{ minHeight: "100dvh", padding: "clamp(16px, 4vw, 40px) clamp(12px, 3vw, 24px) 80px" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button onClick={exportCSV} className="btn-primary" style={{ fontSize: "0.85rem" }}>
                ⬇ Exporter CSV
              </button>
              <button onClick={() => { setSecret(""); setData(null); }} className="btn-ghost" style={{ fontSize: "0.85rem" }}>
                Déconnexion
              </button>
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 24 }}>
          <div className="glass" style={{ borderRadius: 18, padding: "18px 20px", textAlign: "center" }}>
            <p style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.1em",
              textTransform: "uppercase", color: "var(--muted)", marginBottom: 6 }}>Total votants</p>
            <p style={{ fontSize: "2rem", fontWeight: 900, color: "var(--text)", margin: 0 }}>
              <span style={{ color: "#2563EB" }}>{data?.total ?? 0}</span>
            </p>
          </div>
          {Object.entries(byPromo).sort().map(([promo, count]) => (
            <div key={promo} className="glass" style={{ borderRadius: 18, padding: "18px 20px", textAlign: "center" }}>
              <p style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.1em",
                textTransform: "uppercase", color: "var(--muted)", marginBottom: 6 }}>{promo}</p>
              <p style={{ fontSize: "2rem", fontWeight: 900, color: "var(--text)", margin: 0 }}>
                <span style={{ color: "#2ABFC4" }}>{count}</span>
              </p>
            </div>
          ))}
        </div>
        ...
        <div style={{ position: "relative", marginBottom: 16 }}>
          ...
          <input type="text" placeholder="Rechercher par prénom ou promo…"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map((v, idx) => (
            <div key={v.id} className="glass" style={{ borderRadius: 18, padding: "16px 20px" }}>
              {/* Row header */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{
                    ...
                    background: "linear-gradient(135deg, #2563EB40, #2ABFC440)",
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: "0.72rem", color: "var(--muted)", margin: 0 }}>
                      {v.promoType} · {v.voterCategory}
                    </p>
                  </div>
                </div>
                <span style={{ fontSize: "0.72rem", color: "var(--muted)" }}>
                  {new Date(v.votedAt).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })}
                </span>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}>
                ...
                <div style={{ padding: "12px 14px", borderRadius: 12,
                  ...
                  <p style={{ fontSize: "0.62rem", fontWeight: 800, letterSpacing: "0.1em",
                    textTransform: "uppercase", color: "#4890E8", margin: "0 0 8px" }}>
                    Projets
                  </p>
                        ...
                        return (
                          <div key={pv.projectId} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                            <span style={{
                              ...
                              background: proj ? `${proj.color}30` : "rgba(255,255,255,0.1)",
                              color: proj?.color ?? "var(--text)",
                            }}>{pv.rank}</span>
                            <span style={{ flex: 1, fontSize: "0.8rem", fontWeight: 700, color: "var(--text)",
                              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {proj?.name ?? pv.projectId}
                            </span>
                            <span style={{ fontSize: "0.7rem", fontWeight: 800, color: "#4890E8", flexShrink: 0 }}>
                              {pv.weight}pts
                            </span>
                          </div>
                        );
                ...
                {/* ONGs */}
                <div style={{ padding: "12px 14px", borderRadius: 12,
                  background: "rgba(42,191,196,0.06)", border: "1px solid rgba(42,191,196,0.18)" }}>
                  <p style={{ fontSize: "0.62rem", fontWeight: 800, letterSpacing: "0.1em",
                    textTransform: "uppercase", color: "#2ABFC4", margin: "0 0 8px" }}>
                    ONGs
                  </p>
                  {v.ongVotes.length > 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                      {v.ongVotes.map(ov => {
                        ...
                        return (
                          <div key={ov.ongId} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                            ...
                            <span style={{ fontSize: "0.7rem", fontWeight: 800, color: "#2ABFC4", flexShrink: 0 }}>
                              {ov.weight}pts
                            </span>
                          </div>
                        );
          ...
          {filtered.length === 0 && (
            <div className="glass" style={{ borderRadius: 18, padding: "40px", textAlign: "center" }}>
              <p style={{ fontSize: "0.9rem", color: "var(--muted)" }}>
                {(data?.total ?? 0) === 0 ? "Aucun vote enregistré pour l'instant." : "Aucun résultat pour cette recherche."}
              </p>
            </div>
          )}
        </div>
